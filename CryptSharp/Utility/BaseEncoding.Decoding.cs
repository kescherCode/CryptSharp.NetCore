...
public partial class BaseEncoding
{
    ...
    public int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex, int charCount)
    {
        ...
        var byteEnd = checked(byteIndex + byteCount);

        var bitStartOffset = 0;
        for (var i = 0; i < charCount; i++)
        {
            byte value;

            var thisByte = byteIndex + 0 < byteEnd ? bytes[byteIndex + 0] : (byte)0;
            var nextByte = byteIndex + 1 < byteEnd ? bytes[byteIndex + 1] : (byte)0;

            var bitEndOffset = bitStartOffset + BitsPerCharacter;
            if (MsbComesFirst)
            {
