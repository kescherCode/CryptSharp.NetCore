...
public class LdapCrypter : Crypter
{
    ...
    public override string GenerateSalt(CrypterOptions? options)
    {
        ...
        switch (options!.GetValue(CrypterOption.Variant, LdapCrypterVariant.SSha))
        {
            case LdapCrypterVariant.Crypt:
                var crypter = options.GetValue<Crypter>(LdapCrypterOption.Crypter);
                if (crypter is null)
                {
                ...
                var crypterOptions = options.GetValue(LdapCrypterOption.CrypterOptions, CrypterOptions.None);
    ...
    private static string GenerateSaltString() => Convert.ToBase64String(Security.GenerateRandomBytes(8));
    ...
    private static string UnsaltedCrypt(HashAlgorithm algorithm, byte[] password)
    {
        try
        {
            ...
            var crypt = Convert.ToBase64String(algorithm.Hash!);
            return crypt;
        }
        finally
        {
    ...
    private static string SaltedCrypt(HashAlgorithm algorithm, byte[] password, string saltString)
    {
        ...
        try
        {
            ...
            var hashLength = algorithm.HashSize / 8;
            var saltOffset = salt!.Length < hashLength ? 0 : hashLength;
            var saltLength = salt.Length - saltOffset;

            var saltedHash = new byte[hashLength + saltLength];
            try
            {
                algorithm.Initialize();
                algorithm.TransformBlock(password, 0, password.Length, password, 0);
                ...
                var crypt = Convert.ToBase64String(saltedHash);
                return crypt;
            }
            finally
            {
    ...
    private static Regex CreateRegex(string pattern) =>
        new(pattern, RegexOptions.CultureInvariant | RegexOptions.Singleline);

    private static Regex CreateSaltedRegex(string scheme) =>
        CreateRegex(@"\A(?<prefix>{(?i:" + Regex.Escape(scheme) + @")\})(?<salt>[A-Za-z0-9+/]*={0,3})\z");

    private static Regex CreateUnsaltedRegex(string scheme) =>
        CreateRegex(@"\A(?<prefix>{(?i:" + Regex.Escape(scheme) + @")\})[A-Za-z0-9+/]*={0,3}\z");
