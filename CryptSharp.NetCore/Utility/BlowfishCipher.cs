...
public partial class BlowfishCipher : IDisposable
{
    ...
    {
        var magicBytes = Encoding.ASCII.GetBytes(BCryptMagic);
        Array.Resize(ref magicBytes, (magicBytes.Length + 7) / 8 * 8);

        Magic = new uint[(magicBytes.Length + 3) / 4];
        for (var i = 0; i < Magic.Length; i++) { Magic[i] = BitPacking.UInt32FromBEBytes(magicBytes, i * 4); }
    }
    ...
    private BlowfishCipher(uint[]? p, uint[][]? s)
    {
        ...
        var s1 = s ?? S0;
        P = (uint[])(p1 ?? P0).Clone();
        S = new[] { (uint[])s1[0].Clone(), (uint[])s1[1].Clone(), (uint[])s1[2].Clone(), (uint[])s1[3].Clone() };
    }

    /// <summary>
    ///     The number of bytes returned by <see cref="BlowfishCipher.BCrypt()" />.
    /// </summary>
    public static int BCryptLength => (Magic.Length * 4) - 1;
    ...
    {
        Security.Clear(P);
        for (var i = 0; i < S.Length; i++) { Security.Clear(S[i]); }
    }
    ...
    public static BlowfishCipher CreateEks(byte[] key, byte[] salt, int cost) =>
        CreateEks(key, salt, cost, EksBlowfishKeyExpansionFlags.None);
    ...
    public static byte[] BCrypt(byte[] key, byte[] salt, int cost) =>
        BCrypt(key, salt, cost, EksBlowfishKeyExpansionFlags.None);
    ...
    public static byte[] BCrypt(byte[] key, byte[] salt, int cost,
        EksBlowfishKeyExpansionFlags flags)
    {
        using var fish = CreateEks(key, salt, cost, flags);
        return fish.BCrypt();
    }
    ...
    public byte[] BCrypt()
    {
        ...
        try
        {
            ...
            for (var j = 0; j < magicWords.Length; j += 2)
            {
                for (var i = 0; i < 64; i++)
                {
    ...
    private void ExpandKey(byte[] key, byte[] salt, EksBlowfishKeyExpansionFlags flags)
    {
        ...
        for (i = 0; i < p.Length; i++)
        {
            ...
            for (k = 0; k < 4; k++)
            {
                ...
                {
#pragma warning disable CS0675 // Bitwise-or operator used on a sign-extended operand. This is precisely the bug implemented here.
                    data = (data << 8) | (uint)(sbyte)key[j];
#pragma warning restore CS0675 // Bitwise-or operator used on a sign-extended operand. This is precisely the bug implemented here.
                }
                else
                {
                    data = (data << 8) | key[j];
        ...
        var saltL0 = BitPacking.UInt32FromBEBytes(salt, 0);
        var saltR0 = BitPacking.UInt32FromBEBytes(salt, 4);
        var saltL1 = BitPacking.UInt32FromBEBytes(salt, 8);
        var saltR1 = BitPacking.UInt32FromBEBytes(salt, 12);
    ...
    public void Encipher(byte[] buffer, int offset) => Encipher(buffer, offset, buffer, offset);
        ...
        byte[] outputBuffer, int outputOffset)
    {
        ...
        var xl = BitPacking.UInt32FromBEBytes(inputBuffer, inputOffset + 0);
        var xr = BitPacking.UInt32FromBEBytes(inputBuffer, inputOffset + 4);
    ...
    public void Decipher(byte[] buffer, int offset) => Decipher(buffer, offset, buffer, offset);
