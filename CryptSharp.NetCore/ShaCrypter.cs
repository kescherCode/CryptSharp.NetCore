...
public abstract class ShaCrypter : Crypter
{
    ...
    public override string GenerateSalt(CrypterOptions? options)
    {
        ...
        return CryptPrefix
               + (rounds is { } ? string.Format("rounds={0}$", rounds) : "")
    ...
    public override string Crypt(byte[] password, string salt)
    {
        ...
        var match = GetRegex().Match(salt);
        if (!match.Success) { throw Exceptions.Argument("salt", "Invalid salt."); }

        var roundsString = match.Groups["rounds"].Value;
        var roundsStringPresent = roundsString.Length != 0;
        var rounds = roundsStringPresent ? int.Parse(roundsString) : 5000;
    ...
    private byte[] Crypt(byte[] key, byte[] salt, int rounds, HashAlgorithm A)
    {
        ...
        try
        {
            ...
            var permutation = GetCryptPermutation();
    ...
    protected static Regex CreateDefaultRegex(string cryptPrefix, int keyCharacters)
    {
        ...
        var regex = @"\A"
                    ...
                    + keyCharacters
                    + @"}))?\z";
        return new(regex, RegexOptions.CultureInvariant);
    }
    ...
    private static void AddToDigestRolling(HashAlgorithm algorithm, byte[] buffer, int offset, int inputCount,
        int outputCount)
    {
        int count;
