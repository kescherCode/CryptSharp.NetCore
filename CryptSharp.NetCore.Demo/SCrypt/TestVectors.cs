...
namespace CryptSharp.NetCore.Demo.SCryptTest;
...
internal static class TestVectors
{
        ...
        int r, int p, int len, string expected)
    {
        ...
        var derivedBytes = SCrypt.ComputeDerivedKey
        ...
        if (expected != derived)
        {
            Console.WriteLine("WARNING: SCrypt failed test ({0} instead of {1})", derived, expected);
        }
    }
