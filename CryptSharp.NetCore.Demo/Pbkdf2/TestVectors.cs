...
namespace CryptSharp.NetCore.Demo.Pbkdf2Test;
...
internal static class TestVectors
{
    public static void Test()
    {
        ...
        TestFile("CryptSharp.NetCore.Demo.Pbkdf2.TestVectors-PBKDF2.txt");
        ...
        string expected)
    {
        ...
        var keyBytes = Encoding.ASCII.GetBytes(password);
        var saltBytes = Encoding.ASCII.GetBytes(salt);
        var derivedBytes = Pbkdf2.ComputeDerivedKey(new HMACSHA1(keyBytes), saltBytes, c, len);
        ...
        expected = expected
            ...
            .Replace("\t", "")
            .ToUpper();
        var derived = Base16Encoding.Hex.GetString(derivedBytes);
        if (expected != derived)
        {
    ...
    private static void TestFile(string filename)
    {
        ...
        while ((line = reader.ReadLine()!) is { })
        {
            ...
            var derivedKey = Pbkdf2.ComputeDerivedKey(new HMACSHA256(key), salt, iterations, 128);
